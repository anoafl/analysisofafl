...
```
So once we have the box-score statistics from footywire, `fitzRoy::player_stats` we have to aggregate them to a match level. That is because the paper looked at team level statistics not player level. 
...
The next step in the paper is feature creation. What this means is that we are creating some variables to be fed in the model. In this paper while it uses data available from [fitzRoy](https://github.com/jimmyday12/fitzRoy) it also has some varaibles created. These variables are:
