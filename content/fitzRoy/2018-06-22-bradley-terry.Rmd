...
The problem is that this is over-parametized which means that its exactly the same if we were to add a fixed constant to all the values of $\beta{i}$ this is a common issue when rating teams and why sometimes you might see a base team being set. For example if we made it such that $\beta_{Adelaide} =0 $ this means that for every other team $\beta_{j}$ represents the log odds of beating adelaide. 
