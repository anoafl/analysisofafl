...
So lets get the top 10 ruckman in 2016 and see how they stack up their versions of themselves today. 
...
df3<-df2%>%
  ...
  group_by(Season)%>%
  summarise(ave.SC=mean(SC))%>%
      ggplot(aes(x=ave.SC, y=Player)) +geom_point(aes(colour=as.factor(df3$Season)))+ geom_line(aes(group = Player))
