...
```{r}
df2%>%filter(Match_id %in% c("5662","5742","5880","6133","6242"))%>%filter(Team=="Melbourne")%>%filter(HO>0) %>%arrange(desc(HO))
...
```{r}
df1%>%filter(Opposition=="Western Bulldogs")%>%filter(HO>0)%>%ggplot(aes(x=Date, y=SC, label=Player))+geom_point()+geom_label()

```

What is the o/u for Max Gawn if you were a betting man?
