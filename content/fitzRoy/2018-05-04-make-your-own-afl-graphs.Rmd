...
# Anothe example a little explanation
...
```{r}
df%>%
ggplot(aes(x=Weight, y=Contested.Marks)) +
...
`aes`-- Aesthetic mappings describe how variables in the data are mapped to visual properties (aesthetics)  of geoms. Aesthetics are things such as xy and colours `x=Weight, y=Contested.Marks` `colour=Playing.For`
