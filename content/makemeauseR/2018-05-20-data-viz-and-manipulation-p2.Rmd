...
It doesn't look like our dataset has changed, but if we look closely we have a new `Groups:   Season [121]` and hopefully what you have noticed is that our dataset `afldata` has 121 unique values for `Season`. Which we can check below. 
...
So the best way would be if we had some sort of ID variable. Which luckily in `afldata` we have an ID column. 
